...
type BridgeResponse = Partial<MediaMonkeyStatus> & {
  requestId?: number;
  ok: boolean;
  error?: string;
};
...
export class MediaMonkeyBridge {
  ...
  public initialize(): void {
    ...
    const scriptPath = fileURLToPath(
      new URL("./mediaMonkeyAutomationHost.ps1", import.meta.url),
    );
    const child = spawn(
      "powershell.exe",
      [
  ...
  public async getStatus(): Promise<MediaMonkeyStatus | null> {
